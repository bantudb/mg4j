 ...
 * returned by the second operand will be enlarged to the left and to the right by the given amount (e.g.,
...
public class InclusionDocumentIterator extends AbstractIntervalDocumentIterator implements DocumentIterator {
	 ...
	 */
	protected InclusionDocumentIterator( final DocumentIterator first, final DocumentIterator second, final int leftMargin, final int rightMargin ) {
		...
		if ( first.indices().size() != 1 || second.indices().size() != 1 ) throw new IllegalArgumentException( "You can compute the inclusion of single-index iterators only" );
	 ...
	 * @param first the first operand.
	 * @param second the second operand.
	 * @param leftMargin a margin that will be added to the left of each interval returned by the second operand.
	 * @param rightMargin a margin that will be added to the right of each interval returned by the second operand.
	 */
	public static DocumentIterator getInstance( final DocumentIterator first, final DocumentIterator second, final int leftMargin, final int rightMargin ) {
		return new InclusionDocumentIterator( first, second, leftMargin, rightMargin );
	}
	...
	protected IntervalIterator getIntervalIterator( final Index index, final int n, final boolean allIndexIterators, final Object arg ) {
		return new InclusionIntervalIterator();
	}
	...
	public IntervalIterator intervalIterator() throws IOException {
		...
		else if ( firstIntervalIterator == IntervalIterators.TRUE ) return currentIterator = IntervalIterators.TRUE;
		else if ( secondIntervalIterator == IntervalIterators.TRUE ) return currentIterator = IntervalIterators.FALSE;
	...
	protected class InclusionIntervalIterator implements IntervalIterator {
		...
		public Interval nextInterval() throws IOException {
			...
			for( Interval interval; ( interval = firstIntervalIterator.nextInterval() ) != null; ) {
				while( lastSecondInterval.left - leftMargin <= interval.left ) {
					if ( lastSecondInterval.right + rightMargin >= interval.right ) return interval;
