...
public class ContainmentDocumentIterator extends MockDocumentIterator {
	...
	public static MockDocumentIterator getInstance( final DocumentIterator firstIterator, final DocumentIterator secondIterator, final int leftMargin, final int rightMargin ) throws IOException {
		return new ContainmentDocumentIterator( firstIterator, secondIterator, leftMargin, rightMargin );
	}

	static int call = -1;
	protected ContainmentDocumentIterator( final DocumentIterator firstIterator, final DocumentIterator secondIterator, final int leftMargin, final int rightMargin ) throws IOException {
		...
		for ( long documentPointer: it1.elements.keySet() ) {
			...
			if ( it2.elements.keySet().contains( documentPointer ) ) {
				...
				if ( set2 == TRUE ) {
					...
					else for ( Interval interval1: set1 ) 
						addIntervalForDocument( documentPointer, index1, Interval.valueOf( interval1.left, interval1.right ) );
					continue;
				}
				if ( set1 == TRUE ) {  
					addFalseIteratorDocument( documentPointer, index1 );
				...
				for ( Interval interval1: set1 ) {
					...
					for ( Interval interval2: set2 ) 
						if ( Interval.valueOf( interval1.left - leftMargin, interval1.right + rightMargin ).contains( interval2 ) ) {
							good = true;
							break;
						}
